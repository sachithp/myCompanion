...
// Returns a persona only when it belongs to the requesting user
function ownedPersona(db, personaId, userId) {
  return db.prepare('SELECT * FROM personas WHERE id = ? AND user_id = ?').get(personaId, userId)
}

// ── GET /api/personas — list all for this user ────────────────────────────────
router.get('/', (req, res) => {
  const db = getDb()
...
router.post('/', (req, res) => {
  ...
  db.prepare(`
    INSERT INTO personas (
      id, user_id, name, relationship, description, past_conversations, photo_path,
      ocean_openness, ocean_conscientiousness, ocean_extraversion,
      ocean_agreeableness, ocean_neuroticism,
      location, usual_places, daily_routine, interests, likes, dislikes, context_notes
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    id, req.user.userId, name.trim(), relationship || null, description || null,
...
// ── POST /api/personas/import — restore from exported JSON ───────────────────
// Must be placed before /:id routes to prevent Express matching "import" as an ID
...
router.get('/:id', (req, res) => {
  const db      = getDb()
  const persona = ownedPersona(db, req.params.id, req.user.userId)
...
router.put('/:id', (req, res) => {
  const db = getDb()
  if (!ownedPersona(db, req.params.id, req.user.userId)) {
...
router.delete('/:id', (req, res) => {
  ...
  db.prepare('DELETE FROM personas WHERE id = ? AND user_id = ?').run(req.params.id, req.user.userId)
...
router.post('/:id/memories', (req, res) => {
  ...
  res.status(201).json(db.prepare('SELECT * FROM memory_cards WHERE id = ?').get(id))
})

// ── DELETE /api/personas/:personaId/memories/:memoryId ───────────────────────
router.delete('/:personaId/memories/:memoryId', (req, res) => {
  const db = getDb()
  if (!ownedPersona(db, req.params.personaId, req.user.userId)) {
    return res.status(404).json({ error: 'Persona not found' })
  }
  db.prepare(
...
// ── POST /api/personas/:id/relations ─────────────────────────────────────────
router.post('/:id/relations', (req, res) => {
  ...
  res.status(201).json(db.prepare('SELECT * FROM persona_relations WHERE id = ?').get(id))
})
...
router.post('/:id/knowledge', async (req, res) => {
  ...
  res.status(201).json(db.prepare('SELECT * FROM persona_knowledge WHERE id = ?').get(id))
})
...
// ── PUT /api/personas/:id/mode-behaviors/:mode ────────────────────────────────
router.put('/:id/mode-behaviors/:mode', (req, res) => {
  ...
  const text = req.body.behavior?.trim() || ''
  if (text) {
    db.prepare(`
      INSERT INTO persona_mode_behaviors (persona_id, mode, behavior)
      VALUES (?, ?, ?)
...
router.get('/:id/export', (req, res) => {
  ...
  const mode_behaviors = Object.fromEntries(
    db.prepare('SELECT mode, behavior FROM persona_mode_behaviors WHERE persona_id = ?')
      .all(req.params.id)
      .map((r) => [r.mode, r.behavior])
  )
